...
module.exports = {
  connect: function(server) {;
    ...
    .on('connection', function(socket) {
      ...
      //   we create one and emit a 'userupdate' message to the sender so that
      //   a login cycle can complete. We ensure the client id is passed back
      //   so the client can correlate the user, although this is not stored
      //   in Mongo.
      // In all cases, the user is marked as 'online' and the updated chatter
      // list is emitted to all clients (including the original sender).
      //
      socket.on('adduser', function(user_map) {
        ...
        crud.read(
          ...
          function(result_list) {
            var result_map;
            var user_exists = (result_list && result_list.length > 0);
      ...
      // arg_map should have the following properties:
      //   dest_id = id of recipient
      //   dest_name = name of recipient
      //   sender_id = id of sender
      //   msg_text = message text
      //
      // If the recipient is online, the arg_map is emitted to him.
      // If not, a 'user has gone offline' message is emitted to the sender.
      //
      socket.on('updatechat', function(arg_map) {
        if (chatter_map.hasOwnProperty(arg_map.dest_id)) {
          chatter_map[arg_map.dest_id].emit('updatechat', arg_map);
        }
        else {
      ...
      // 'updateavatar' message handler
      // arg_map should have the following properties:
      //   person_id = the id of the persons avatar to update
      //   css_map   = the css map for avatar position and color
      // This routine updates the entry in Mongo, and then emits the revised
      // the revised people list to all clients.
      //
      socket.on('updateavatar', function(arg_map) {
        crud.update(
          'users',
