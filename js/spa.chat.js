...
spa.chat = (function() {
	...
	setSliderPosition = function(position_type, callback) {
		...
		// therefore return false; the shell will fix the URI and try again.
		...
		// Prepare the animate parameters
		...
		case 'closed':
			...
			toggle_text = '+';
			break;
		// Executed if position_type encountered is unknown
		default:
			return false;
		}
	...
	onUpdatechat = function(event, msg_map) {
		var is_user;
		var sender_id = msg_map.sender_id;
		var msg_text  = msg_map.msg_text;
		var chatee    = configMap.chat_model.get_chatee() || {};
		var sender    = configMap.people_model.get_by_cid(sender_id);
	...
	// Actions:
	//    If the window height or width falls below a given threshold, resize
	//    the chat slider for the reduced window size.
	// Returns: Boolean
	...
	handleResize = function() {
		// If no slider container exists, do nothing.
		if (!jqueryMap.$slider) {
			return false;
		}
	...
	removeSlider = function() {
		...
		// Remove DOM container; this removes event bindings also
		if (jqueryMap.$slider) {
	...
	//    with the provided arguments. No other actions are taken.
	//    Expected arguments for this module:
	//    * set_chat_anchor - a method to modify the URI anchor to indicate
	//      opened or closed state. Return false if requested state cannot
	//      be met.
	//    * chat_model - the chat model object provides methods to interact
	//      with our instant messaging.
	//    * people_model - the people model object which provided methods
	//      to interact with the list of users the model maintains.
	//    * slider_* settings. All these area optional scalars.
	...
	// Purpose:
	//    Directs module to offer its feature to the user.
	// Arguments:
	//    * $append_target - the jQuery DOM element where we will append the
	//      slider div
	// Action:
	//    Appends a slider div to the provided container and filles it with
	//    the chat HTML content. It them initializes elements, events, and
	...
	initModule = function($append_target) {
	    ...
	    $.gevent.subscribe(jqueryMap.$list_box, 'spa-listchange', onListchange );
	    $.gevent.subscribe(jqueryMap.$list_box, 'spa-setchatee',  onSetchatee  );
	    $.gevent.subscribe(jqueryMap.$list_box, 'spa-updatechat', onUpdatechat );
	    $.gevent.subscribe(jqueryMap.$list_box, 'spa-login',      onLogin      );
	    $.gevent.subscribe(jqueryMap.$list_box, 'spa-logout',     onLogout     );
